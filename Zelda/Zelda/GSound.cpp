...
int GSound::Load(const wstring& _RelativeFilePath)
{
	if (nullptr == GSoundManager::GetInst()->GetSoundDevice())
		assert(nullptr);

	wstring strContent = GPathManager::GetContentPath();
	strContent += _RelativeFilePath;
...
bool GSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(GSoundManager::GetInst()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
void GSound::PlayToBGM(bool _bLoop)
{
	GSoundManager::GetInst()->RegisterToBGM(this);
...
void GSound::SetVolume(float _fVolume)
{
	m_iVolume = GetDecibel(_fVolume);
	m_pSoundBuffer->SetVolume(m_iVolume);
}

void GSound::SetPosition(float _fPosition)
{
	Stop(true);
...
int GSound::GetDecibel(float _fVolume)
{
	if (_fVolume > 100.f)
		_fVolume = 100.f;
